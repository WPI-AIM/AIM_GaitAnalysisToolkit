...
def animate(frame):
    ...
    T_RTh = markers.get_frame("LowerLegs_Healthy:RightThigh")[frame]
    T_RSh = markers.get_frame("LowerLegs_Healthy:RightShank")[frame]
    T_RF = markers.get_frame("LowerLegs_Healthy:RightFoot")[frame]
    T_LTh = markers.get_frame("LowerLegs_Healthy:LeftThigh")[frame]
    T_LSh = markers.get_frame("LowerLegs_Healthy:LeftShank")[frame]
    T_LF = markers.get_frame("LowerLegs_Healthy:LeftFoot")[frame]
    T_H = markers.get_frame("LowerLegs_Healthy:Hip")[frame]


    x = [T_H[:,-1][0], T_RTh[:,-1][0], T_RSh[:,-1][0], T_RF[:,-1][0], T_LTh[:,-1][0], T_LSh[:,-1][0], T_LF[:,-1][0]]
    y = [T_H[:,-1][1], T_RTh[:,-1][1], T_RSh[:,-1][1], T_RF[:,-1][1], T_LTh[:,-1][1], T_LSh[:,-1][1], T_LF[:,-1][1]]
    z = [T_H[:,-1][2], T_RTh[:,-1][2], T_RSh[:,-1][2], T_RF[:,-1][2], T_LTh[:,-1][2], T_LSh[:,-1][2], T_LF[:,-1][2]]
    arrow_prop_dict = dict(mutation_scale=5, arrowstyle='->', shrinkA=0, shrinkB=0)

    ax.clear()
    for coor in [T_H, T_RTh, T_RSh, T_RF, T_LTh, T_LSh, T_LF]:
        og = coor[:,-1]
        axis_x = coor[:, 0]
        axis_y = coor[:, 1]
        axis_z = coor[:, 2]
